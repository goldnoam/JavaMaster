...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-300 transition-colors duration-300">
      {/* Header */}
      <header className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white py-4 px-6 sticky top-0 z-30 shadow-lg flex justify-between items-center backdrop-blur-md bg-opacity-90 dark:bg-opacity-90">
        <div className="flex items-center gap-3">
          <div className="bg-orange-600 p-2 rounded-xl shadow-lg shadow-orange-900/20">
            <Coffee size={24} className="text-white" />
          </div>
          <div>
            <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-400 bg-clip-text text-transparent">Java Mastery Hub</h1>
        ...
        <div className="relative hidden md:block group">
          ...
          <input 
            ...
            className="bg-white dark:bg-slate-950 rounded-xl py-2 pl-10 pr-10 w-80 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50 transition-all border border-slate-200 dark:border-slate-800 placeholder:text-slate-400"
          />
          {searchQuery && (
            <button 
              onClick={handleClearSearch}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-white p-0.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              <X size={14} />
            </button>
          )}
        ...
        <div className="flex gap-3 items-center">
          <button 
            onClick={toggleTheme}
            className="p-2.5 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all active:scale-95"
            title="Toggle Theme"
          >
          ...
          <button 
            ...
            className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-700 dark:text-slate-300 transition-all border border-slate-300 dark:border-slate-700 active:scale-95 shadow-sm"
          >
      ...
      <main className="flex-1 flex flex-col lg:flex-row max-w-[1600px] mx-auto w-full">
        ...
        <aside className="w-full lg:w-80 bg-slate-50 dark:bg-slate-900/50 border-r border-slate-200 dark:border-slate-800 overflow-y-auto max-h-[50vh] lg:max-h-[calc(100vh-72px)] lg:sticky lg:top-[72px] flex flex-col scrollbar-thin">
          <div className="p-6 pb-0">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
                    selectedCategory === cat 
                      ...
                      : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600 hover:text-slate-700 dark:hover:text-slate-200'
                  }`}
                >
                  {cat}
                </button>
            ...
            <h2 id="curriculum" className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-4">Mastery Syllabus</h2>
          </div>
                ...
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl transition-all flex items-center gap-4 group border ${
                    selectedTopic.id === topic.id 
                      ? 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white shadow-md ring-1 ring-orange-500/30' 
                      : 'bg-transparent border-transparent text-slate-500 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-slate-700 dark:hover:text-slate-300'
                  }`}
                >
                  <div className={`p-2 rounded-xl transition-colors ${selectedTopic.id === topic.id ? 'bg-slate-100 dark:bg-slate-900' : 'bg-slate-100 dark:bg-slate-900/50'}`}>
                    {getCategoryIcon(topic.category)}
                  </div>
                  <div className="flex-1 truncate">
                    <div className="text-sm font-bold truncate">{topic.title}</div>
                    <div className="text-[10px] font-mono text-slate-400 dark:text-slate-600 tracking-wider uppercase mt-0.5">{topic.version || topic.category}</div>
                  </div>
                  <ChevronRight size={16} className={`text-slate-300 dark:text-slate-700 group-hover:text-slate-500 dark:group-hover:text-slate-400 transition-all transform ${selectedTopic.id === topic.id ? 'translate-x-1 text-orange-500' : ''}`} />
                </button>
              ))
            ) : (
        ...
        <section ref={contentSectionRef} className="flex-1 p-6 md:p-12 lg:p-16 bg-white dark:bg-slate-950">
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-3 mb-6">
              <span className="px-3 py-1 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[10px] font-bold rounded-lg border border-indigo-500/20 uppercase tracking-widest">
                {selectedTopic.category}
              </span>
              {selectedTopic.version && (
                <span className="px-3 py-1 bg-orange-500/10 text-orange-600 dark:text-orange-400 text-[10px] font-bold rounded-lg border border-orange-500/20 uppercase tracking-widest">
                  {selectedTopic.version}
                </span>
              )}
            </div>

            <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-6 tracking-tight leading-tight">{selectedTopic.title}</h2>
            <p className="text-xl text-slate-600 dark:text-slate-400 mb-12 leading-relaxed font-medium">
              {selectedTopic.description}
            </p>

            <div className="bg-slate-50 dark:bg-slate-900 rounded-3xl shadow-xl dark:shadow-2xl overflow-hidden mb-12 border border-slate-200 dark:border-slate-800 shadow-indigo-500/5">
              <div className="bg-white dark:bg-slate-800/80 px-6 py-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-700/50">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-slate-400 dark:text-slate-400 uppercase tracking-[0.2em] font-bold">Source.java</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button 
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-slate-950 hover:bg-slate-100 dark:hover:bg-slate-900 text-[10px] font-bold text-slate-700 dark:text-slate-300 transition-all uppercase tracking-widest border border-slate-300 dark:border-slate-800 active:scale-95"
                  >
              ...
              <div className="relative group">
                <pre className="p-8 text-sm font-mono overflow-x-auto text-slate-800 dark:text-indigo-200 leading-relaxed scrollbar-thin bg-white dark:bg-slate-900/50">
              ...
              {(terminalOutput || isRunning) && (
                <div ref={terminalRef} className="border-t border-slate-200 dark:border-slate-800 bg-slate-100/90 dark:bg-black/80 backdrop-blur animate-in slide-in-from-top-2 duration-300">
                  <div className="px-6 py-2 bg-slate-200/50 dark:bg-slate-900/50 flex items-center justify-between border-b border-slate-300 dark:border-slate-800/50">
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-500 font-bold uppercase tracking-widest flex items-center gap-2">
                      <Terminal size={12} />
                      Output Console
                    </span>
                    <button onClick={() => setTerminalOutput(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-400">
                      <X size={14} />
                    </button>
                  </div>
                  <div className="p-6 font-mono text-sm min-h-[100px]">
                    {isRunning ? (
                      ...
                      <div className="space-y-1 text-emerald-600 dark:text-emerald-400">
                        <div className="text-slate-500 dark:text-slate-600 mb-2 font-bold text-[10px]">EXECUTION RESULT:</div>
            ...
            <div className="mb-12">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 leading-relaxed font-medium shadow-inner relative overflow-hidden">
              ...
              <div className="mb-16 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <div className="space-y-4">
                  {selectedTopic.versionHistory.map((update, idx) => (
                    <div key={idx} className="bg-slate-50 dark:bg-slate-900/40 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row gap-4 group hover:bg-slate-100 dark:hover:bg-slate-800/40 transition-all">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed group-hover:text-slate-800 dark:group-hover:text-slate-300 transition-colors">
                        {update.description}
                      </p>
                    </div>
                  ))}
      ...
      <footer className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 py-16 px-6">
        <div className="max-w-6xl mx-auto flex flex-col items-center text-center gap-12">
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <span className="text-lg font-black text-slate-900 dark:text-white tracking-tight">Java Mastery Hub</span>
              <p className="text-sm text-slate-500 dark:text-slate-500 mt-1">Refining the Java ecosystem since 2025.</p>
            </div>
          </div>
